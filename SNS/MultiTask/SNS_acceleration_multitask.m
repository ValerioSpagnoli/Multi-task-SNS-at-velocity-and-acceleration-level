...
function q_ddot_SNS = SNS_acceleration_multitask(m, n, J, J_dot, q_dot, x_ddot, bounds_max, bounds_min, verbose)
    ...
    for k=1:l
        ...
        while limit_exceeded
            ...
            q_ddot_bar_k = q_ddot_tilde_k + pinv(J{k}*P_bar_k)*(s_k*x_ddot{k}-J_dot{k}*q_dot-J{k}*q_ddot_tilde_k);

            q_ddot_bar{length(q_ddot_bar)+1} = q_ddot_bar_k;

            if verbose
            fprintf('q_ddot_bar_N_k = \n');disp(q_ddot_bar_N_k);
            fprintf('q_ddot_tilde_k = \n');disp(q_ddot_tilde_k);
            fprintf('q_ddot_bar_k = \n');disp(q_ddot_bar_k);
            end
        
            for i=1:n
                if q_ddot_bar_k(i) > bounds_max(i) || q_ddot_bar_k(i) < bounds_min(i)
                    limit_exceeded = true;
                end
            end
            ...
            if limit_exceeded
                ...
                if task_scaling_factor > s_star_k
                    ...
                    fprintf('    s_stark_k = %f\n', s_star_k);
                    fprintf('    W_star_k = \n');disp(W_star_k);
                    fprintf('    q_ddot_star_N_k = \n');disp(q_ddot_star_N_k);
                ...
                W_k(j,j) = 0;
                if q_ddot_bar_k(j) > bounds_max(j)
                    q_ddot_N_k(j) = bounds_max(j)-q_ddot_bar_km1(j);
                elseif q_ddot_bar_k(j) < bounds_min(j)
                    q_ddot_N_k(j) = bounds_min(j)-q_ddot_bar_km1(j);
                end
